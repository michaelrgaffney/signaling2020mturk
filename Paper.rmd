...
```{r cdc, fig.width=8, fig.cap="US Suicidality non-fatal injury and death rates by age and sex (2001-2019)."}
...
As both a validity check and a way to understand the degree participants would be willing to help if they believed the participant completely, we presented participants with strong evidence that the claims were true (*T3 Action*). In the US sample, this involved telling participants there was video evidence of the event in question occurring or a similar event after the fact. With the thwarted marriage vignette, this involved the participant seeing the man their daughter wants to marry trash talk their daughter and their family.
...
```{r beliefeffect, fig.width=6, fig.height=8, fig.cap=paste0("The effect of the signals on *T2 Belief* (top) and *T2 Action* (bottom) controlling for *T1 Belief* and *T1 Action*, respectively. Effects plotted for T1 values set to their median value (*T1 Belief*=", median(d$T1Belief, na.rm=T), ", *T1 Action*=", median(d$T1Action, na.rm=T), "). Fit using generalized linear regression models with the quasibinomial family. Grey bars are 95% CIs. For coefficents, p-values, and other statistics, see models m1 and m3 in Tables \\@ref(tab:regressiontables) and \\@ref(tab:anovatables).")}
...
The within-subject effects of each signal are depicted in Figure \@ref(fig:rawbeliefactions2d).
...
Our results suggest that the costs of depression and suicidality, which in mainstream theories are taken as evidence of brain dysfunction, might instead function to help victims elicit support from reluctant social partners with whom they are in conflict.
...
The `glm` models we report in the main text were:

`r extract_eq(models$Model$m1, intercept='beta', wrap=T)` `r extract_eq(models$Model$m3, intercept='beta', wrap=T)` `r extract_eq(models$Model$m1b, intercept='beta', wrap=T)` `r extract_eq(models$Model$m3b, intercept='beta', wrap=T)`
