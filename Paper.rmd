...
Major depression and suicide are generally viewed as pathological responses to stress that elicit aversive responses from others. Alternatively, depression and suicide attempts might involve honest signals of need in response to stress and help to bargain for increased support from reluctant others. To address this hypothesis, we examined responses to emotional signals in an experimental vignette study involving claims of substantial need in the presence of conflicts of interest and private information about whether the signaler was actually in need. In our sample of 1,240 participants recruited from Amazon Mechanical Turk, costlier signals like depression and suicide attempts resulted in greater perceptions of need and increased likelihood of support compared to simple verbal requests and crying without further symptoms. Additionally, the effect of signal type on likelihood of support was largely mediated by the effect of signaling on belief in need as predicted. Our results suggest that depression and suicide attempts, apparent human universals, are relatively successful ways of signaling honest need and eliciting support, consistent with costly signaling explanations.

# Introduction

Conflicts of interest are ubiquitous, even among closely related individuals. In species that rely heavily on cooperation, like our own, conflicts can arise over the levels of investment in a cooperative endeavor and the division of the resulting benefits. In many such cases, individuals who were dissatisfied with the terms of cooperation could seek other partners [@hammerstein2016]. In many other cases, however, it would be difficult or impossible to switch partners. Spouses who were dissatisfied with their partner's investment in their new infant, for example, would not easily find a different partner to invest in that infant [@Hagen:1999]. Similarly, an adolescent who was dissatisfied with her parent's investment in her would not easily find other parents who were willing to invest more, nor could parents easily produce another adolescent. In many cooperative endeavors central to human biological fitness, each party has monopoly power over the benefits they bring to the endeavor -- no one is easily replaced [@Hagen2003].

@Hagen2003 proposed that physical aggression and depression were complementary strategies to resolve conflicts in interdependent relationships. @SellEtAl2009 found that physically formidable individuals were more prone to anger, prevailed more in conflicts of interest, and considered themselves entitled to better treatment. Physically weaker individuals, though, are not without options to resolve conflicts in their favor. An individual with monopoly power over the benefits she contributes to a critical cooperative endeavor can withhold those benefits, or put them at risk, until her partners change their behaviors in ways that benefit her.

Withholding or risking one's contributions to a cooperative endeavor can be conceptualized as credibly signaling private information about one's need. Cooperation generates benefits over time. Thus, withholding or risking contributions incurs a *cost of delay*. In a classic non-cooperative game theory model of bargaining over the division of benefits, @Rubenstein:1982 showed that two parties can come to an immediate agreement over division of benefits despite conflicts of interest if the parties' valuations of cooperation are not private information. If valuations are private information, however, then immediate agreement might not be possible because each party has an incentive to exaggerate their need. A willing to delay, i.e., incur a cost, is a credible signal that one has a low valuation of the endeavor, and is therefore genuinely in need. Once valuations are known, the parties can reach an agreement on the division of benefits [@KennanWilson1993; @feinberg2005].

<!-- Introduce the types of social situations most strongly associated to depression and suicide here, maintaining the emphasis on conflict -->
...
## Anthropology of depression and suicide

Anthropology, in contrast, has long viewed depressive states as largely the result of social problems. Early in the field’s history, anthropologists tackled the problem of suicide as manifested in the small, traditional societies in which they conducted their fieldwork. Suicide, they found, was commonly a form of protest, revenge, and/or appeal (e.g., Malinowski, 1932; Niehaus, 2012). Some ethnographers, such as Hezel (1987), conceptualized suicide as a form of anger, whereas others, such as Giddens (1964), conceptualized suicide as a form of social pressure. Counts (1980) emphasized the powerlessness of suicide victims. Common to almost all theoretical and empirical investigations of suicide is a focus on completed suicides, i.e., suicide deaths.

Raymond Firth, in contrast, viewed suicidality as a gamble to improve one's circumstances in the here and now. Based on observations that suicide attempts often followed loss or conflict and varied substantially in their likelihood of death, he argued that a sizable subset of the suicide attempts among the Tikopia were not meant to end in death but instead were a means to elicit attention, status, or immediate reintegration into the community following negative events (Firth 1936; 1961). More recently, Syme, Garfield, & Hagen (2016) also emphasized that the vast majority of suicidal behavior does not result in death, arguing the proper focus should be on suicide ideation and attempts, with death as an unintended consequence. See Figure 1.

<!-- move figure one here -->
...
Participants in the US sample were randomly assigned to either the "basketball coach," "romantic partner," or "brother-in-law" vignettes. In the "basketball coach" scenario, the star player on a university basketball team comes to the participant (who is asked to imagine themselves as the athletic director at a university) and claims she was sexually assaulted by her coach. However, there are indications the player might be lying to get more playing time.
...
Those in the Indian sample only saw the "thwarted marriage vignette." In this scenario, the participant and their family are described as trying to arrange a dowry for their older daughter so she can marry a man she already loves. After the man's family demands more money, the participant's family tries to find a second man, who the older daughter claims to find unattractive. Due to the younger daughter in the story only being two years younger, any additional investment in the dowry will come at the younger daughter's expense.
...
The current study is a refinement of a large MTurk experimental vignette pilot study (N=`r nrow(signalingdata2018b)`) that used a different vignette but very similar signals and outcomes, and a much smaller pilot study posted to [reddit.com/r/SampleSize/](https://www.reddit.com/r/SampleSize/) (N=28) that used draft versions of 3 vignettes used here, along with the same signals and outcomes. In the MTurk pilot study, there was a single vignette in which an imagined sister claims her child needs an expensive emergency medical procedure, and asks for a very large loan. In the small reddit.com study, participants viewed and responded to two vignettes used here, each with a different signal.
...
`r extract_eq(models_prereg$Model$m1, intercept='beta', wrap=T)` `r extract_eq(models_prereg$Model$m3, intercept='beta', wrap=T)`
...
`r extract_eq(models_prereg$Model$m1b, intercept='beta', wrap=T)` `r extract_eq(models_prereg$Model$m3b, intercept='beta', wrap=T)`
...
`r extract_eq(med_prereg$model.m, intercept='beta', wrap=T)` `r extract_eq(med_prereg$model.y, intercept='beta', wrap=T)`
...
: (\#tab:outcomes) Outcome variables in each vignette.
...
`r extract_eq(models$Model$m1, intercept='beta', wrap=T)` `r extract_eq(models$Model$m3, intercept='beta', wrap=T)`
...
`r extract_eq(med1$model.m, intercept='beta', wrap=T)` `r extract_eq(med1$model.y, intercept='beta', wrap=T)`
...
Who to thank: Lab group? Arpita? Committee? Anne sounds like she will provide a lot of feedback, so her after that?
...
Angry Sad Suicidal Mentally ill Depressed Guilty\
Calm Neutral Scared Tired Distressed Devious Jealous Confident Traumatized Violated None of the above
...
hagenheat(d_diff, rotate_labels = F, seriation_method = 'PCA_angle') + scale_fill_gradient2(low='blue', high='red')
...
```{r feedbackplot, fig.width=9, fig.cap="The distribution of optional feedback provided at the end of the study. None: no feedback provided (includes, e.g., 'none', 'no', 'N/A'). Generic: A few short words, such as 'nice study', or 'thanks'. Informative: one or two sentences commenting on specific aspects of the survey."}

hagenutils::ggxtabs(xtabs(~Feedback3 + vignette, d), cell_counts = T) + xlab('')
```

```{r informativeplot, fig.width=9, fig.cap="The distribution of informative feedback provided at the end of the study. See Figure \\@ref(fig:feedbackplot) for other types of feedback. See Table \\@ref(tab:informativetab) for the text of all informative statements."}
